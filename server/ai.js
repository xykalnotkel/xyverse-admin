...
/* ---------- pemanggil dasar ---------- */
async function groqChat({ system, user, model, temperature = 0.7, max_tokens = 4000, json = false }) {
  if (!aktif()) {
    const e = new Error('GROQ_API_KEY belum diatur. Tambahkan di file .env lalu jalankan ulang server.');
    e.status = 400;
    throw e;
  }
  ...
  if (!r.ok) {
    ...
    try { pesan = JSON.parse(t).error?.message || pesan; } catch {}
...
router.get('/ai/status', async (_req, res) => {
  res.json({
    aktif: aktif(),
    model: MODEL_DEFAULT,
    pesan: aktif() ? 'Groq terhubung' : 'GROQ_API_KEY belum diatur di file .env',
  });
});

/* ---------- daftar model (live dari Groq) ---------- */
router.get('/ai/models', async (_req, res) => {
  if (!aktif()) return res.json({ models: MODEL_FALLBACK, live: false });
  try {
    ...
    const models = (d.data || [])
      ...
      .sort();
    res.json({ models: models.length ? models : MODEL_FALLBACK, live: true });
  } catch {
    res.json({ models: MODEL_FALLBACK, live: false });
  }
});

/* ---------- 1. usulkan ide judul ---------- */
router.post('/ai/ide', async (req, res, next) => {
  try {
    ...
    const { teks, pakai } = await groqChat({
      model, temperature: 0.9, max_tokens: 1200, json: true,
...
Judul harus spesifik dan menggugah rasa ingin tahu, bukan generik.`,
    ...
    res.json({ ...ambilJSON(teks), pakai });
  } catch (e) { next(e); }
});

/* ---------- 2. tulis konten lengkap ---------- */
router.post('/ai/tulis', async (req, res, next) => {
  try {
    ...
    res.json({ body: isi, pakai, model: dipakai });
  } catch (e) { next(e); }
});

/* ---------- 3. buat metadata dari isi ---------- */
router.post('/ai/meta', async (req, res, next) => {
  try {
    ...
    const { teks, pakai } = await groqChat({
      model, temperature: 0.3, max_tokens: 900, json: true,
...
/* ---------- 4. perbaiki / ubah gaya tulisan ---------- */
router.post('/ai/perbaiki', async (req, res, next) => {
  try {
    const { body = '', mode = 'rapikan', model } = req.body;
    if (!body.trim()) return res.status(400).json({ error: 'Isi konten masih kosong' });
    ...
    const { teks, pakai } = await groqChat({
      model, temperature: 0.6, max_tokens: 6000,
      system: `${BRAND}\n\nKeluarkan HANYA hasil Markdown-nya. Tanpa komentar atau penjelasan.`,
      user: `${tugas}\n\n---\n${body}`,
    });

    const isi = teks.replace(/^```(?:markdown|md)?\n?/i, '').replace(/\n?```$/,'').trim();
    res.json({ body: isi, pakai });
  } catch (e) { next(e); }
});

/* ---------- 5. buat draf sekaligus (judul + meta + isi) ---------- */
router.post('/ai/draf-penuh', async (req, res, next) => {
  try {
    const { col = 'blog', topik = '', panjang = 'sedang', model } = req.body;
    if (!topik.trim()) return res.status(400).json({ error: 'Topik wajib diisi' });

    // tahap 1: judul
    const { teks: t1 } = await groqChat({
      model, temperature: 0.9, max_tokens: 400, json: true,
    ...
    // tahap 2: isi
    const r2 = await fetch(`http://127.0.0.1:${process.env.PORT || 4500}/api/ai/tulis`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ col, judul, panjang, model }),
    }).then((r) => r.json());

    // tahap 3: metadata
    const r3 = await fetch(`http://127.0.0.1:${process.env.PORT || 4500}/api/ai/meta`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ col, judul, body: r2.body, model }),
    }).then((r) => r.json());

    res.json({ judul, body: r2.body, meta: r3 });
  } catch (e) { next(e); }
});
