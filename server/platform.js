...
const SESSION_ONLY = /^\/api\/(team|inbox|audit|settings|deploy)(\/|$)/;
...
export function izin(req, res, next) {
  ...
  if (SESSION_ONLY.test(req.urlPath) && req.admin.jenis !== 'sesi') return json(res, 403, { error: 'Hanya akun tim melalui sesi browser.' });
...
export function pasangPlatform(r) {
  ...
  r.jalan('POST','/api/team',async(req,res)=>{
    ...
    await db.audit(req.admin.pengguna,'team_created',id);
  ...
  r.jalan('PATCH','/api/team/:id',async(req,res)=>{
    ...
    if(req.body?.reset===true){
      ...
      await db.audit(req.admin.pengguna,'team_password_reset',u.id);
    ...
    await db.audit(req.admin.pengguna,req.body.aktif?'team_enabled':'team_disabled',u.id);
  ...
  r.jalan('PATCH','/api/inbox/:id',async(req,res)=>{
    ...
    await db.audit(req.admin.pengguna,'inbox_updated',req.params.id);json(res,200,{ok:true});
  });
